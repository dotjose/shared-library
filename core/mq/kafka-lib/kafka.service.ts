...
import { Consumer, Kafka, Producer, Admin, SeekEntry, TopicMessages } from "kafkajs";
...
export class KafkaService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(
    ...
    this.kafka = new Kafka({
      ...this.config.client,
      logCreator: this.kafkaLogger.getKafkaJSLoggingAdapter.bind(this.kafkaLogger),
    });
  ...
  private async connectToSchemaRegistry(): Promise<void> {
    const eventEmitterTopics = this.kafkaEventFunctionsService.getEventEmitterTopics() ?? [];
    ...
    if (this.producer) {
      await this.kafkaAvroSerializer.initialize(this.config.schemaRegistry, eventEmitterTopics);
    }
    if (this.consumer) {
      const sTopics = this.kafkaEventFunctionsService.getEventHandlerTopics() ?? [];
      let subjectProbe: string = undefined;
      if (sTopics.length > 0) {
        subjectProbe = getSchemaRegistryValueSubjectByTopic(sTopics[Math.floor(Math.random() * sTopics.length)]);
      }
    }
  }
  ...
  private async fetchTopicOffsets(): Promise<void> {
    for await (const topic of this.kafkaEventFunctionsService.getEventHandlerTopics()) {
      try {
        ...
        this.kafkaLogger.error(`Error while fetching topic offset for ${topic}`, reject);
        // throw reject;
      }
    }
  }
  ...
  private async subscribeToTopics(): Promise<void> {
    ...
    for await (const topic of this.kafkaEventFunctionsService.getEventHandlerTopics()) {
      try {
        ...
        this.kafkaLogger.error(`Error while subscribing to topic ${topic}`, reject);
      }
    }
  }
    ...
    this.consumer.run({
      ...
      eachMessage: async ({ topic, message }) => {
        try {
          ...
          await retry(
            async () => {
              const event = await this.kafkaAvroDeserializer.deserialize(message);
              await this.kafkaEventFunctionsService.callEventHandler(topic, event);
            },
            5,
            this.config?.consumer?.retry?.retries ?? 2,
            (reason) => {
              this.kafkaLogger.warn(`Retrying to process message on topic ${topic}.`, reason);
            }
          );
        } catch (reject) {
          this.kafkaLogger.error(`Error while processing message on topic ${topic}.`, reject);
        }
      },
    });
  }
  ...
  async emit<V, K>(
    ...
    if (!this.producer) {
      this.kafkaLogger.error(
        `Cannot emit event(s) on topic(s) ${
          Array.isArray(payload) ? payload.map((p) => p.topic).join(", ") : payload.topic
        }`
      );
      return;
    }
    try {
      ...
      await this.producer.send({
        ...
        messages: [
          {
            value: JSON.stringify(payload.event),
            key: new Date().valueOf().toString(),
          },
        ],
      });
      this.kafkaLogger.log(`${payload.topic} event sent successfully `);
    } catch (reject) {
      this.kafkaLogger.error(`Error while emitting event(s): ${JSON.stringify(payload)}`, reject);
      if (!failSilent) {
        // throw reject;
      }
    }
