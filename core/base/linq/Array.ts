...
Array.prototype.aggregate = function <T, TAccumulate, TResult>(
  ...
  return resultSelector(
    this.reduce(
      (accumulator: TAccumulate, element: T) => func(accumulator, element),
      seed
    )
  );
};
...
Array.prototype.average = function <T>(
  ...
  return (
    (selector !== undefined ? this.sum(selector) : this.sum()) / this.length
  );
};
...
Array.prototype.elementAtOrUndefined = function <T>(
  this: T[],
  index: number
): T | undefined {
  if (index < 0 || index >= this.length) {
...
Array.prototype.max = function <T>(
  ...
  return selector !== undefined
    ? this.reduce(
        (accumulator: number, element: T) =>
          Math.max(selector(element), accumulator),
        Number.MIN_VALUE
      )
...
Array.prototype.min = function <T>(
  ...
  return selector !== undefined
    ? this.reduce(
        (accumulator: number, element: T) =>
          Math.min(selector(element), accumulator),
        Number.MAX_VALUE
      )
    : this.reduce((accumulator: number, element: T) => {
        if (typeof element === "number") {
...
Array.prototype.selectMany = function <T, TCollection, TResult>(
  ...
  return this.reduce(
    (accumulator: TResult[], currentValue: T, currentIndex: number) => [
      ...accumulator,
      ...collectionSelector(currentValue, currentIndex).map(
        (value: TCollection) => resultSelector(currentValue, value)
      ),
    ],
    []
  );
...
Array.prototype.singleOrUndefined = function <T>(
  this: T[],
  predicate?: (element: T) => boolean
): T | undefined {
  if (this.length < 1) {
...
Array.prototype.sum = function <T>(
  ...
  return selector !== undefined
    ? this.reduce(
        (accumulator: number, element: T) => accumulator + selector(element),
        0
      )
    : this.reduce((accumulator: number, element: T) => {
        if (typeof element === "number") {
...
Array.prototype.toDictionary = function <T, TKey, TElement>(
  ...
  return new Map(
    this.map((element: T) => [keySelector(element), elementSelector(element)])
  );
};
