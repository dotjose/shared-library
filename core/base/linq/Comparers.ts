...
export const strictEqualityComparer =
  <T>() =>
  (left: T, right: T) =>
    left === right;

export function combineComparers<T>(
  left: Comparer<T>,
  right: Comparer<T>
): Comparer<T> {
...
export function createComparer<TElement, TKey>(
  ...
  ascending: boolean,
  customComparer?: Comparer<TKey>
): Comparer<TElement> {
  if (customComparer !== undefined) {
    return (l: TElement, r: TElement) =>
      customComparer(keySelector(l), keySelector(r));
  }
