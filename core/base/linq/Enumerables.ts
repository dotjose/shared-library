...
export interface IQueryable<TOut> {
  ...
  aggregate<TValue>(
    aggregator: Aggregator<TOut, TValue>,
    initialValue: TValue
  ): TValue;
  ...
  select<TSelectorOut>(
    selector: Selector<TOut, TSelectorOut>
  ...
  sequenceEqual(
    other: IQueryable<TOut> | TOut[],
    comparer: EqualityComparer<TOut>
  ): boolean;
  ...
  zip<TOther, TSelectorOut>(
    other: IQueryable<TOther> | TOther[],
    selector: ZipSelector<TOut, TOther, TSelectorOut>
  ): IEnumerable<TSelectorOut>;
}
...
export abstract class EnumerableBase<TElement, TOut>
  implements IEnumerable<TOut>
{
  ...
  public sequenceEqual(
    other: IQueryable<TOut> | TOut[],
    comparer: EqualityComparer<TOut> = strictEqualityComparer<TOut>()
  ): boolean {
    const otherEnumerable =
    ...
    while (this.next()) {
      if (
        !otherEnumerable.next() ||
        !comparer(this.value(), otherEnumerable.value())
      ) {
        return false;
      }
    }
  ...
  public selectMany<TSelectorOut>(
    ...
    const selectToEnumerable = (e: TOut) => {
      ...
      return Array.isArray(ie) ? new ArrayEnumerable(ie) : ie.asEnumerable();
    };

    return this.select(selectToEnumerable)
      .toArray()
      .reduce(
        (p, c) => new ConcatEnumerable(p, c),
        Enumerable.empty()
      ) as IEnumerable<TSelectorOut>;
  }
  ...
  public concat(
    ...
    const asEnumerable = (e: TOut[] | IQueryable<TOut>): IIterable<TOut> => {
      return e instanceof Array ? new ArrayEnumerable(e) : e.asEnumerable();
    };

    let result = new ConcatEnumerable<TOut>(
      this.copy(),
      asEnumerable(other).copy()
    );

    for (let i = 0, end = others.length; i < end; ++i) {
      result = new ConcatEnumerable<TOut>(
        result,
        asEnumerable(others[i]).copy()
      );
    }
  ...
  groupBy<TKey extends Indexer, TValue>(
    ...
    for (let i = 0; i < array.length; ++i) {
      ...
      const value =
        valueSelector !== undefined ? valueSelector(array[i]) : array[i];
  ...
  public min<TSelectorOut>(
    ...
    return this.aggregate((previous, current) =>
      previous !== undefined && previous < current ? previous : current
    );
  }
  ...
  public orderBy<TKey>(
    ...
    return new OrderedEnumerable(
      this.copy(),
      createComparer(keySelector, true, comparer)
    );
  }
  ...
  public orderByDescending<TKey>(
    ...
    return new OrderedEnumerable(
      this.copy(),
      createComparer(keySelector, false, undefined)
    );
  }

  public max(): TOut;
  public max<TSelectorOut>(
    selector: Selector<TOut, TSelectorOut>
  ): TSelectorOut;
  public max<TSelectorOut>(
    selector?: Selector<TOut, TSelectorOut>
  ...
  public sum(selector: Selector<TOut, number>): number {
    return this.aggregate(
      (previous: number, current: TOut) => previous + selector(current),
      0
    );
  }
  ...
  public zip<TOther, TSelectorOut>(
    other: IQueryable<TOther> | TOther[],
    selector: ZipSelector<TOut, TOther, TSelectorOut>
  ): IEnumerable<TSelectorOut> {
    const otherAsEnumerable =
      other instanceof Array
        ? new ArrayEnumerable(other)
        : other.asEnumerable();

    return new ZippedEnumerable<TOut, TOther, TSelectorOut>(
      this,
      otherAsEnumerable,
      selector
    );
...
export class Enumerable<TElement> extends EnumerableBase<TElement, TElement> {
  ...
  public static fromSource<TElement>(
    source: TElement[] | IIterable<TElement>
...
export class ConditionalEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public copy(): ConditionalEnumerable<TElement> {
    return new ConditionalEnumerable<TElement>(
      this.source.copy(),
      this._predicate
    );
  }
...
export class SkipWhileEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public copy(): SkipWhileEnumerable<TElement> {
    return new SkipWhileEnumerable<TElement>(
      this.source.copy(),
      this._predicate
    );
  }
...
export class TakeWhileEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public copy(): TakeWhileEnumerable<TElement> {
    return new TakeWhileEnumerable<TElement>(
      this.source.copy(),
      this._predicate
    );
  }
...
export class ConcatEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public copy(): ConcatEnumerable<TElement> {
    return new ConcatEnumerable<TElement>(
      this.source.copy(),
      this._otherSource.copy()
    );
  }
...
export class UniqueEnumerable<TElement, TKey> extends Enumerable<TElement> {
  ...
  private isUnique(element: TElement): boolean {
    const key =
      this._keySelector !== undefined ? this._keySelector(element) : element;
...
export class RangeEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public copy(): RangeEnumerable<TElement> {
    return new RangeEnumerable<TElement>(
      this.source.copy(),
      this._start,
      this._count
    );
  ...
  private isValidIndex(): boolean {
    ...
    return (
      ...
      (end === undefined || this._currentIndex < end)
    );
  }
...
export class TransformEnumerable<TElement, TOut> extends EnumerableBase<
  TElement,
  TOut
> {
  protected source: IEnumerable<TElement>;
  ...
  public copy(): TransformEnumerable<TElement, TOut> {
    return new TransformEnumerable<TElement, TOut>(
      this.source.copy(),
      this._transform
    );
  }
...
export class ReverseEnumerable<TElement> extends Enumerable<TElement> {
  ...
  public max(): TElement;
  public max<TSelectorOut>(
    selector: Selector<TElement, TSelectorOut>
  ): TSelectorOut;
  public max<TSelectorOut>(
    selector?: Selector<TElement, TSelectorOut>
  ): TElement | TSelectorOut {
    if (selector !== undefined) {
  ...
  public value(): TElement {
    ...
    return this._elements.value[
      this._elements.value.length - 1 - this._currentIndex
    ];
  }
}
// endregion
...
export class ArrayEnumerable<TOut> extends Enumerable<TOut> {
  ...
  public aggregate<TValue>(
    ...
    return this.list.aggregate(
      aggregator as Aggregator<TOut, TOut | undefined>
    );
  }
...
export class DefaultIfEmptyEnumerable<TOut> extends EnumerableBase<
  TOut,
  TOut | undefined
> {
  ...
  public copy(): DefaultIfEmptyEnumerable<TOut> {
    return new DefaultIfEmptyEnumerable<TOut>(
      this.source.copy(),
      this._defaultValue
    );
  }
  ...
  public next(): boolean {
    ...
    this._mustUseDefaultValue =
      this._mustUseDefaultValue === undefined && !hasNextElement;
...
export class ZippedEnumerable<TElement, TOther, TOut> extends EnumerableBase<
  TElement,
  TOut
> {
  ...
  public constructor(
    source: IIterable<TElement>,
    otherSource: IIterable<TOther>,
    selector: ZipSelector<TElement, TOther, TOut>
  ) {
    super(source);
  ...
  public copy(): ZippedEnumerable<TElement, TOther, TOut> {
    return new ZippedEnumerable<TElement, TOther, TOut>(
      this.source.copy(),
      this._otherSource.copy(),
      this._selector
    );
  ...
  public value(): TOut {
    if (!this._currentValue.isValid()) {
      this._currentValue.value = this._selector(
        this.source.value(),
        this._otherSource.value()
      );
    }
  ...
  public next(): boolean {
    ...
    if (!this._isOneOfTheSourcesFinished) {
      this._isOneOfTheSourcesFinished =
        !super.next() || !this._otherSource.next();
    }
