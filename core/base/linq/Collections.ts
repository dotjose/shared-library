  ...
  implements IQueryable<TElement>
{
  ...
  public select<TSelectorOut>(
    ...
    return new TransformEnumerable<TElement, TSelectorOut>(
      this.asEnumerable(),
      selector
    );
  }
  ...
  public selectMany<TSelectorOut>(
    selector: Selector<
      TElement,
      TSelectorOut[] | List<TSelectorOut> | IEnumerable<TSelectorOut>
    >
  ): IEnumerable<TSelectorOut> {
    const selectToEnumerable = (e: TElement) => {
      const ie = selector(e);

      return ie instanceof Array ? new ArrayEnumerable(ie) : ie.asEnumerable();
    };
  ...
  public sequenceEqual(
    other: IQueryable<TElement> | TElement[],
    comparer: EqualityComparer<TElement>
  ): boolean;
  public sequenceEqual(
    other: IQueryable<TElement> | TElement[],
    comparer?: EqualityComparer<TElement>
  ...
  public sum(selector: Selector<TElement, number>): number {
    return this.aggregate(
      (previous: number, current: TElement) => previous + selector(current),
      0
    );
  }
  ...
  public aggregate(
    aggregator: Aggregator<TElement, TElement | undefined>
  ): TElement;
  public aggregate<TValue>(
    aggregator: Aggregator<TElement, TValue>,
    initialValue: TValue
  ): TValue;
  public aggregate<TValue>(
  ...
  ): IEnumerable<TElement | undefined> {
  ...
  public zip<TOther, TSelectorOut>(
    ...
    selector: ZipSelector<TElement, TOther, TSelectorOut>
  ): IEnumerable<TSelectorOut> {
...
export abstract class ArrayQueryable<
  TElement
> extends EnumerableCollection<TElement> {
  ...
  public sequenceEqual(
    other: IQueryable<TElement> | TElement[],
    comparer: EqualityComparer<TElement> = strictEqualityComparer<TElement>()
  ): boolean {
    if (other instanceof ArrayQueryable || other instanceof Array) {
      const thisArray = this.asArray();
      const otherArray =
        other instanceof ArrayQueryable
  ...
  implements IDictionary<TKey, TValue>
{
  ...
  public static fromJsObject<TValue = string>(
    ...
    const keyValues = keys.select(
      (k) => <IKeyValue<string, TValue>>{ key: k, value: object[k] }
    );
  ...
  public toArray(): Array<IKeyValue<TKey, TValue>> {
    return this.getKeys()
      .select<IKeyValue<TKey, TValue>>((p) => {
        return {
          key: p,
  ...
  public getKeys(): IList<TKey> {
    ...
    return new List(
      keys.map((k) => (this.keyType === "number" ? parseFloat(k) : k)) as TKey[]
    );
  }
